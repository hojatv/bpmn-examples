...
public class SpringTest extends AbstractTest {
	...
	public void simpleProcessTest() {
		...
		variableMap.put("isbn", 123456L);
		runtimeService.startProcessInstanceByKey("bookorder", variableMap);
		List<Task> task = taskService.createTaskQuery().orderByTaskName().asc().list();
		assertEquals("Complete order", task.get(task.size()-1).getName());
		taskService.complete(task.get(task.size()-1).getId());
		assertEquals(0, runtimeService.createProcessInstanceQuery().count());
		*/
	}
}
